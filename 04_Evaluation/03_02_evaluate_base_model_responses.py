#!/usr/bin/env python3
"""
Script to evaluate Samantha mental health counseling responses using GPT-4.1-mini and Google Gemini Flash 2.5.
Evaluates responses based on 7 consultation metrics with detailed scoring.

Supports JSONL input format with Samantha structure:
- User questions in 'question' field
- AI responses in 'answer' field
"""
...
class SamanthaResponseEvaluator:
...
HOLISTIC_APPROACH: [1-10]"""
    ...
    def load_samantha_jsonl(self, jsonl_file: str) -> pd.DataFrame:
        """Load Samantha JSONL file and extract user questions and AI responses."""
        data = []
        ...
        with open(jsonl_file, 'r', encoding='utf-8') as f:
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    # Extract user question from 'question' field
                    if 'question' in json_obj:
                        user_input = json_obj['question']
                    else:
                        print(f"Warning: Line {line_num} missing 'question' field")
                        continue
                    
                    # Extract AI response from 'answer' field
                    if 'answer' in json_obj:
                        ai_output = json_obj['answer']
                    else:
                        print(f"Warning: Line {line_num} missing 'answer' field")
                        continue
                    ...
                    entry = {
                        ...
                        'model': json_obj.get('model', 'samantha'),
                        'original_language': json_obj.get('original_language', 'unknown'),
                        'line_number': json_obj.get('line_number', line_num),
                        'tokens_prompt': json_obj.get('tokens_prompt', 0),
                        'tokens_completion': json_obj.get('tokens_completion', 0),
                        'model_path': json_obj.get('model_path', 'unknown')
                    }
        ...
        print(f"Loaded {len(df)} valid entries from Samantha JSONL file")
        return df

    def load_data(self, file_path: str, sample_size: int = None) -> pd.DataFrame:
        """Load the data from Samantha JSONL file and optionally sample."""
        if not file_path.endswith('.jsonl'):
            raise ValueError(f"This evaluator only supports JSONL files. Got: {file_path}")
        
        df = self.load_samantha_jsonl(file_path)
    ...
    def evaluate_single_response(self, row_index: int, user_input: str, ai_response: str, metadata: Dict = None) -> Dict:
...
def main():
    ...
    # Configuration for Samantha results
    input_file = "/home/taitran/mq/mq-nlp-group/4_Evaluation/samantha_results_samantha-1.2_20250616_130855.jsonl"
    output_file = "/home/taitran/mq/mq-nlp-group/4_Evaluation/evaluation_results_samantha_model_v1_2.csv"
    stats_file = "/home/taitran/mq/mq-nlp-group/4_Evaluation/evaluation_statistics_samantha_model_v1_2.json"
    sample_size = 50   # Start with smaller sample, adjust based on your needs and API limits
    ...
    print("Samantha Mental Health Response Evaluation System")
    print("=" * 60)
    ...
    try:
        ...
        evaluator = SamanthaResponseEvaluator(openai_api_key, gemini_api_key)
        ...
        print(f"Model: {df.iloc[0].get('model', 'N/A')}")
        print(f"Category: {df.iloc[0].get('category', 'N/A')}")
        ...
        if stats:
            ...
            print(f"\nTop Metrics (Combined Average):")
            metric_names = ['active_listening', 'empathy_validation', 'safety_trustworthiness', 
                          'openness_nonjudgment', 'clarity_encouragement', 'boundaries_ethical', 'holistic_approach']
            for metric in metric_names:
                score = stats.get(f'avg_{metric}_mean', 0)
                print(f"  {metric.replace('_', ' ').title()}: {score:.2f}")
        
        print(f"\n🎉 Samantha evaluation completed successfully!")
